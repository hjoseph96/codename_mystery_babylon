...
namespace SS.TwoD
{
    public struct SSRect
    {
        ...
        public SSRect(int x, int y, int width, int height)
        {
            this.x = x;
            this.y = y;
            this.width = width;
    ...
    public class SpriteGenerator : MonoBehaviour, IGenerator
    {
        ...
        int m_SpritePerDirection;
        float m_OriginalAnimationDuration;
        ...
        SpriteGeneratorManager.RotateAxis m_RotateAxis;
        ...
        void SetupParameters()
        {
            ...
            if (manager != null)
            {
                ...
                m_RotateAxis = manager.rotateAxis;
                m_ShadowType = (m_RotateAxis == SpriteGeneratorManager.RotateAxis.Y) ? manager.shadowType : SpriteGeneratorManager.ShadowType.NoShadow;
        ...
        void SetupAnimation()
        {
            ...
            if (m_Animator != null)
            {
                UnityEditor.Animations.AnimatorController ac = m_Animator.runtimeAnimatorController as UnityEditor.Animations.AnimatorController;
                ...
                for (int i = 0; i < sm.states.Length; i++)
                {
                    ...
                    if (state.state.name == m_AnimationName)
                    {
                        AnimationClip clip = state.state.motion as AnimationClip;
                        if (clip != null)
                        {
                            float shotSpeed = manager.shotSpeed;
                            m_OriginalAnimationDuration = clip.length;
                            m_SpriteQuantity = Mathf.RoundToInt(m_AnimationFrameRate * m_AnimationDuration);
                            
                            m_SpritePerDirection = 1;
                            if (m_ShadowType == SpriteGeneratorManager.ShadowType.RealShadow) m_SpritePerDirection++;
                            if (HasColor()) m_SpritePerDirection++;
                            
                            int totalSprite = m_SpritePerDirection * m_SpriteQuantity;
                            float shotDuration = (float)totalSprite / shotSpeed;
                            
                            m_AnimatorSpeed = m_OriginalAnimationDuration / shotDuration;
                        }
                        else
                        {
                            float shotSpeed = manager.shotSpeed;
                            m_OriginalAnimationDuration = 1;
                            m_SpriteQuantity = 1;
        ...
        void TakeAnimation()
        {
            ...
            if (m_Animator != null)
            {
                m_Animator.speed = (m_SpriteQuantity > 1) ? m_AnimatorSpeed : 0;
            ...
            {
                float shotDuration = m_OriginalAnimationDuration / m_AnimatorSpeed;
                InvokeRepeating("Take", 0, shotDuration / m_SpriteQuantity);
            }
        }
        
        void Take()
        {
            StartCoroutine(CoTake());
        }
        
        void CastShadowOnly(bool shadowOnly)
        {
            Renderer[] renderers = m_Animator.gameObject.GetComponentsInChildren<Renderer>();
        ...
        IEnumerator CoTake()
        {
            ...
            if (m_Count >= m_SpriteQuantity)
            {
                ...
                float rotateAngle = 360f / m_MaxDirection;
                Vector3 rotateVector = Vector3.zero;
                
                switch (m_RotateAxis) {
                    case SpriteGeneratorManager.RotateAxis.X:
                        rotateVector = new Vector3(rotateAngle, 0, 0);
                        break;
                    case SpriteGeneratorManager.RotateAxis.Y:
                        rotateVector = new Vector3(0, rotateAngle, 0);
                        break;
                    case SpriteGeneratorManager.RotateAxis.Z:
                        rotateVector = new Vector3(0, 0, rotateAngle);
                        break;
                }
                m_RootObject.Rotate(rotateVector);
                ...
                {
                    m_Animator.speed = 1;
                    m_Animator.Play("_Dummy");
                    
                    Invoke("TakeAnimation", 0.5f);
                }
                else
                {
                    #if UNITY_EDITOR
        ...
        {
            int total = m_TexturesBodyOnly.Length * m_SpritePerDirection;
            int count = 0;
        ...
        void SaveSpriteFile()
        {
            for (int i = 0; i < m_TexturesShadowOnly.Length; i++)
            {
            ...
            for (int i = 0; i < m_TexturesBodyOnly.Length; i++)
            {
                ...
                for (int row = 0; row < m_TexturesBodyOnly[i].height; row++)
                {
                    for (int col = 0; col < m_TexturesBodyOnly[i].width; col++)
                    {
                        ...
                        if ((colorsShadowOnly != null && !CompareColor(colorsShadowOnly[idx], maskColor)) || colorsBodyOnly[idx].a > 0)
                        {
                            if (row < minY)
                                minY = row;
                            done = true;
