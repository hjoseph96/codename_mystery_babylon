	...
	public class BufferPresets {
		public static void Draw(BufferPresetList bufferList) {
			...
			for(int i = 0; i < bufferList.list.Length; i++) {
				...
				LayerSettings.DrawList(bufferList.list[i].lightLayers, "Light Layers (" + bufferList.list[i].lightLayers.list.Length  + ")", Lighting2D.Profile.layers.lightLayers, false);
