...
namespace MalbersAnimations.Controller
{
    ...
    { 
        ...
        {
            if (!isActiveAndEnabled ||                                  //Check if the Zone is Active
                other.isTrigger ||                                      //No Triggers
                !MTools.CollidersLayer(other, Layer.Value) ||           //Just accept animal layer only
                HeadOnly && !other.name.Contains(HeadName)) return;     //If is Head Only and no head was found Skip
        ...
        void OnTriggerExit(Collider other)
        {
            if (!isActiveAndEnabled ||                                   //Check if the Zone is Active
                 other.isTrigger ||                                      //No Triggers
                 !MTools.CollidersLayer(other, Layer.Value) ||           //Just accept animal layer only
                  HeadOnly && !other.name.ToLower().Contains(HeadName.ToLower())) return;     //If is Head Only and no head was found Skip
        ...
        private void ActivateModeZone(bool forced)
        {
            ...
            if (forced || automatic)
            {
                if (MTools.ElapsedTime(ZoneActivationTime, AutomaticDisabled))
                {
                    ZoneActivationTime = Time.time;
