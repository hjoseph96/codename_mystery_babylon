    ...
    public partial class MAnimal
    {
        ...
        protected void TryActivateState()
        {
            ...
            foreach (var newState in states)
            {
                ...
                {
                    if (newState.IgnoreLowerStates) return;    //If the Active State cannot exit ignore lower priority States
                    continue;                               //Ignore Try Activating yourSelf
                }

                if (newState.OnActiveQueue && !newState.OnQueue) //Wake UP the State that is no longer on QUEUE and it was activated! (PRIORITY FOR THE  QUEDED STATES)!
                {
                    newState.ActivateQueued();
                    break;
                }

                if ((newState.UniqueID + CurrentFrame) % newState.TryLoop != 0) continue;                   //Check the Performance Loop 
                ...
                {
                    if (newState.Active && !newState.OnExitCoolDown && !newState.IsSleepFromState && !newState.IsSleepFromMode && !newState.OnQueue && newState.TryActivate())
                    {
                        newState.Activate();
                        break;
                    }
