...
namespace MalbersAnimations.Controller
{
    ...
    public class MAnimalAIControl : MonoBehaviour, IAIControl, IAITarget, IAnimatorListener
    {
        ...
        //public IWanderArea IsWanderArea { get; protected set; }
      ...
      //  public bool NullDestination => DestinationPosition == NullVector;
        ...
        public virtual bool TargetIsMoving { get; protected set; }


        /// <summary> Is the Animal waiting x time to go to the Next waypoint</summary>
        public virtual bool IsWaiting { get; protected set; }

        /// <summary>Destination Position to use on Agent.SetDestination()</summary>
        public virtual Vector3 DestinationPosition// { get; protected set; }
        {
        ...
        protected virtual void Update_TargetPos()
        {
            if (UpdateTargetPosition)
            {
                ...
                var DistanceOnMovingTarget = Vector3.Distance(DestinationPosition, Agent.transform.position); //Double check if the Animal is far from the taret
        ...
        public virtual void ClearTarget() => SetTarget(null, false);
        ...
        public virtual void SetTarget(Transform target, bool Move)
        {
           // Debug.Log("Set target = " + target);
            ...
            if (target != null)
            {
                ...
                MoveAgent = Move; //THe Agent will move if the move parameter is true... if not it will only calculate all the logic without moving

                //Resume the Agent is MoveAgent is true
                CheckAirTarget();
                ResumeAgent();
                if (MoveAgent) Debuging($"<color=yellow>is travelling to: <B>{target.name}</B> - {DestinationPosition} </color>");
            }
            else
            {
        ...
        protected virtual void CheckAirTarget()
        {
            ...
            {
                Debuging(": Next Waypoint is AIR", NextTarget.gameObject);
        ...
        public virtual void ResumeAgent()
        {
            if (!FreeMove)
            {
                ...
                if (Agent.isOnOffMeshLink)   Agent.CompleteOffMeshLink(); //Complete OffMesh Link
            }
        }
        ...
        {
            if (IsWaiting) return;      //If we have no were to go then Skip the code
