...
namespace MalbersAnimations.Controller
{
    public class Fall : State
    {
        ...
        public override void TryExitState(float DeltaTime)
        {
            ...
            //GoingDown = Vector3.Dot(animal.DeltaPos, Gravity) > 0; //Check if is falling down
            float DeltaDistance_Down = 0;

            if (GoingDown)
            {
                DeltaDistance_Down = Vector3.Project(animal.DeltaPos, Gravity).magnitude;
            ...
            if (Physics.Raycast(FallPoint, Gravity, out FallRayCast, 100f, animal.GroundLayer, QueryTriggerInteraction.Ignore))
            {
                ...
                //  if (animal.IsOnZone && animal.HasExternalForce) return; //HACK FOR SPRING AND WIND ZONES
            ...
            if (!animal.IsOnZone && !animal.HasExternalForce)
            {
                ...
                {
                    if (LastDeltaDistance > DeltaDistance_Down)
                    {
            ...
            LastDeltaDistance = DeltaDistance_Down;
