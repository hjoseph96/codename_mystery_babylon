...
namespace MalbersAnimations.Controller.Reactions
{
    ...
    public abstract class MReaction : ScriptableObject
    {
        ...
        public void React(GameObject animal) => React(animal.FindComponent<MAnimal>());

        public void React(MonoBehaviour animal)
        {
            if (animal is MAnimal) _React(animal as MAnimal);
        }



        public MonoBehaviour AnimalComponent(MonoBehaviour[] monos)
        {
            foreach (var m in monos) if (m is MAnimal) return m; //Find the animal  Monobehaviour
            return null;
        }
